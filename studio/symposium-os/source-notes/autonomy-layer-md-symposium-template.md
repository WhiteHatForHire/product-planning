---
title: "AUTONOMY LAYER.md Symposium Template"
source_archive: "Symposium Studios"
source_path: "######Symposium Studios/#Studio OS/#SYMPOSIUM OS/AUTONOMY_LAYER.md Symposium Template.docx"
status: active
privacy: working
tags:
  - studio-os
...
# AUTONOMY LAYER.md Symposium Template

> Imported from the source archive and converted to Markdown for searchability. Treat the source path as provenance, not as the current canonical location.
AUTONOMY_LAYER.md — Symposium OS Canonical Edition v2.1
...
MCP commit of large append-only logs — last resort. Prefer creating small per-session run notes at docs/run-notes/session-YYYY-MM-DD-[context].md and referencing them later.
...
Refetches origin and re-detects open PRs in the chain

For each remaining open branch, runs the squash-merge SHA drift check above

Applies --onto rebase against the new main HEAD

Resolves conflicts per the Tier 1/2/3 rules above

Pushes with --force-with-lease

Logs each rebase per the rebase log format above
...
If the operator's workflow requires per-PR review before merge (auto-merge: no), the agent stops after preparing all remaining branches against current main and reports the chain state. The operator merges the next PR. The agent's next invocation resumes from state detection without needing a new directive body.
...
Prompt or AI changes → MANUAL_PLAYTEST_REQUIRED. UI changes → MANUAL_PLAYTEST_REQUIRED. Log all to deferred-issues.md.
...
Push branch. Direct git push preferred. If blocked, MCP write of small files only — apply section 1.12 preflight. Open PR. If Auto-merge: yes, register auto-merge. If no, log URL and stop.
...
End of AUTONOMY_LAYER.md (Symposium OS Canonical Edition) v2.1
