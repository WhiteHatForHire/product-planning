...
- After any backend wave that may have changed request or response shapes
...
2. List endpoints with HTTP method, path, auth requirement, runtime (edge / serverless / node / other).
3. Document request body shape (typed schema if present; otherwise extract from runtime validation, destructuring, or first-line type assertions).
...
Plus a "Spec deltas" section listing each mismatch, with file/line references back to the handler source.
...
- Halt if any endpoint's auth requirement is unclear or unverified. Auth ambiguity at the API boundary is a security risk.

## Related
- `AUTONOMY_LAYER.md` section 1.13 (spec-reality reconciliation)
- `skills/diagnostics/feed-diagnostic` (analogous skill for external data sources)
