...
category: planning
trigger: Before introducing new domain terminology, building a feature with uncertain language, or clarifying ambiguous concepts.
---
...
- Writing a directive that introduces new domain concepts
...
- Before any wave that depends on shared ubiquitous language being unambiguous
...
6. Identify decisions that should become ADRs: hard to reverse, future readers would wonder why, real trade-off involved.
...
## Stop conditions
- Halt if the plan introduces terms that conflict with safety-relevant or anti-vocabulary entries in `CONTEXT.md`. Escalate.
- Halt if more than 5 critical ambiguities surface. The plan needs more design work before directive generation.
